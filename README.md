...
- タイトルにファイル名として使用できない文字（例: \ : * ? " < > | # [ ] など）、または先頭・末尾のスペースやドット、空文字が含まれている場合、そのページはid.mdというファイル名で出力され、元のタイトルはfrontmatterのtitleとして記録されます。
- Obsidian上ではtitleにより元タイトルでの検索・リンクが可能です。
